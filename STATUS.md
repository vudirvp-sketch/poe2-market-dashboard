...
> **Last updated:** 2026-07-13 (iter 144 — `docs/DATA_FLOW.md` §2 + §5 deep cross-check against `backend/data/providers/poe2scout.py` + `src/lib/poe2api.ts`: §2 (endpoint #17 path params aligned to `{CurrencyOneItemId}/{CurrencyTwoItemId}`; note marking #2/#3/#21 as available-but-not-consumed), §5.1 (RawUniqueItem phantom `ApiId` field removed — code at `poe2api.ts:1025` explicitly notes uniques have NO ApiId; `ItemMetadata`/`IsChanceable` types fixed on both Raw interfaces; mapping `id` priority order fixed `ItemId || CurrencyItemId/UniqueItemId`; `apiId` rule split for currencies vs uniques; `relativePrice` fallback-to-currentPrice noted), §5.2 (3 missing fields added `currency1CategoryApiId`/`currency2CategoryApiId`/`currency2RelativePrice`; `relativePrice` coalescing fixed — code does NOT `?? 0`; null-init noted for change/history fields). 13 drift items. 0 new bugs. 1466 pytest green, 0 source-code changes (doc-only iter).)
