...
> **Last updated:** 2026-07-13 (iter 148 — TD-6 phase 2 follow-up SHIPPED. Extended `nameRu` rendering to the remaining 4 UI components that displayed unique-item names in EN only: `comparison-dialog.tsx`, `comparative-chart.tsx`, `leveling-uniques-widget.tsx`, `fuzzy-search.tsx`. All now use `locale === "ru" && item.nameRu ? item.nameRu : item.name` (or `getUniqueDisplayName` for `LevelingUniquesWidget` which uses the backend `LevelingUnique` type without a `nameRu` field). `fuzzy-search.tsx` additionally keeps the EN name as a `nameAlt` fuse.js key (weight 0.25) so users can search across both languages (e.g. EN search "Brynhand" still finds "Клеймо Бринханда" when locale=ru). Closed KI-34 (PairComparisonDialog labels were frozen at add-time — now re-derived from `pair.currency1Id`/`pair.currency2Id` via `getCurrencyDisplayName` on every render). 14 new tests added. **1518 pytest green + 704 Jest green + tsc clean + 0 new ESLint errors.**)
...
**Identified iter 148** while auditing pair-comparison-dialog.tsx for the nameRu extension. The iter-147 worklog listed pair-comparison-dialog as a candidate for the nameRu work, but inspection showed it doesn't render `item.name` at all — it renders `pair.label` (a pre-computed currency pair string). The label WAS already locale-aware at add-time (`exchange-table.tsx:698` and `exchange-pair-card.tsx:81` both build it via `getCurrencyDisplayName(pair.currency1Id, locale)`), but the label was FROZEN in the zustand store. Switching locale after adding pairs to comparison didn't refresh the chip/legend/summary labels.

**Fix (iter 148):** Added a `liveLabel(pair)` helper inside `PairComparisonDialog` that re-derives the label from `pair.currency1Id` / `pair.currency2Id` via `getCurrencyDisplayName(..., locale)` on every render. Falls back to the stored `pair.label` only when `getCurrencyDisplayName` returns null for either currency (e.g. an apiId not in our RU translation map). The query result now stores the full `pair` object (not just `label`) so the live re-derivation works without re-fetching. 2 regression tests added in `src/__tests__/unique-items-i18n.test.tsx:PairComparisonDialog`.
...
**Fix (iter 146):** Added Stage 6 `--apply-audit` flag to `scripts/sync_currency_names_from_poe2db.py`. Reads `scripts/.cache/translation_audit.json` (Stage 5 output), overwrites every `mismatches` entry's value in `currency_names_ru` with the `poe2db_ru` value. Requires `--confirm`. Idempotent. Does NOT touch `currency_names_en`. RU/EN key parity preserved. Ran the apply: **32/32 applied, 0 skipped**. Regenerated TS mirror. Updated `tests/test_currency_names_ru.py:46-51` spot-check for `exalted`. 12 new tests added in `tests/test_sync_currency_names.py`.
...
| **TD-6** | P2 | Translation alignment with poe2db official + unique-items RU support. **Phase 1 SHIPPED iter 146** (`--apply-audit` + 32 KI-32 drift items). **Phase 2 SHIPPED iter 147:** `--fetch-unique-ru` + `--apply-unique` flags; 445 unique items added; `mapUniqueItem` populates `nameRu`; `unique-table.tsx` uses `nameRu` when locale=ru. **Phase 2 follow-up SHIPPED iter 148:** extended `nameRu` rendering to `comparison-dialog`, `comparative-chart`, `leveling-uniques-widget`, `fuzzy-search` (with `nameAlt` cross-locale search); closed KI-34 (PairComparisonDialog label-staleness). **Phase 3:** re-audit cycle (monthly `--audit` + `--apply-audit` for currency; monthly `--fetch-unique-ru` + `--apply-unique` for unique items to pick up new items). |
...
**Plus 1 partial unique item (iter 147 Stage 7):** `Demigods_Virtue` — poe2db has RU (`Добродетель полубога`) but no EN in the index. Skipped by `--apply-unique` (we want both names or neither). Re-run `--fetch-unique-ru` after a poe2db update to pick it up.
...
| Unique items (weapons, armour, accessories) show English names even when locale=ru | **TD-6 phase 2 + follow-up** (SHIPPED iter 147 + iter 148) — `unique_names_ru`/`unique_names_en` sections populated (445 items). `mapUniqueItem` in `poe2api.ts:1030` populates `nameRu` field. ALL UI components that display unique-item names now use `nameRu` when locale=ru: `unique-table.tsx`, `comparison-dialog.tsx`, `comparative-chart.tsx`, `leveling-uniques-widget.tsx` (via `getUniqueDisplayName` since `LevelingUnique` has no `nameRu` field), `fuzzy-search.tsx` (with EN kept as `nameAlt` for cross-locale search). To refresh: `--fetch-unique-ru` + `--apply-unique --confirm` + `python scripts/sync_currency_names_ts.py`. Known limitation: `leveling-uniques-widget` has partial RU coverage because poe2db slugs don't always match the curated names (e.g. "Polcirkeln Sapphire Ring" → slug `Polcirkeln_Sapphire_Ring` doesn't match poe2db slug `Polcirkeln`). Of the 10 leveling uniques, only ~1-2 currently have a poe2db RU match. Full coverage would require a curated `nameRu` field on the backend `LevelingUniqueData` model — deferred. | `src/lib/poe2api.ts:mapUniqueItem`, `src/components/dashboard/{unique-table,comparison-dialog,comparative-chart,leveling-uniques-widget,fuzzy-search}.tsx`, `backend/data/currency_names.json:unique_names_ru/en` |
| PairComparisonDialog shows stale EN/RU labels after switching locale | **KI-34** (FIXED iter 148) — `pair.label` was frozen at add-time. Now re-derived from `pair.currency1Id`/`pair.currency2Id` via `getCurrencyDisplayName(..., locale)` on every render, with the stored `label` as fallback when no RU translation exists. | `src/components/dashboard/pair-comparison-dialog.tsx:liveLabel` |
...
5. `--audit` (iter 145, KI-32 enabler) — READ-ONLY audit of all existing currency RU translations against poe2db current RU `<title>`. Writes `scripts/.cache/translation_audit.json`. Does NOT modify `currency_names.json`.
...
7. `--fetch-unique-ru` (iter 147, TD-6 phase 2) — fetches poe2db unique-item INDEX pages in both RU (`/ru/Unique_item`) and EN (`/us/Unique_item`), parses `<a class="UniqueItem">` anchors, joins on slug, writes `scripts/.cache/poe2db_unique_names.json`. Only 2 HTTP calls (fast). Does NOT modify `currency_names.json`.
8. `--apply-unique --confirm` (iter 147, TD-6 phase 2) — applies unique-item cache to `unique_names_ru`/`unique_names_en` sections in `currency_names.json`. Idempotent. RU/EN parity preserved by construction.

After any JSON update, run `python scripts/sync_currency_names_ts.py` to regenerate the TS mirror (emits `CURRENCY_NAMES_RU/EN`, `CATEGORY_NAMES_RU/EN`, `UNIQUE_NAMES_RU/EN`, and helpers `getCurrencyRuName`/`getCurrencyEnName`/`getCategoryRuName`/`getCategoryEnName`/`getCurrencyDisplayName`/`getCategoryDisplayName`/`getCurrencyPairDisplayName` + `enNameToUniqueSlug`/`getUniqueRuName`/`getUniqueEnName`/`getUniqueDisplayName`). KI-33 fix (iter 145): all URL construction goes through `_build_poe2db_url()` which percent-encodes non-ASCII chars. Unique items have NO ApiId in poe2scout — they're keyed by poe2db URL slug (e.g. `Brynhands_Mark`), derived from the EN name via `enNameToUniqueSlug()` (strip apostrophes, replace spaces with underscores).

**UI nameRu rendering pattern (iter 147 + iter 148).** All components that display a `PoeItem.name` for a unique item should use the locale-aware pattern: `locale === "ru" && item.nameRu ? item.nameRu : item.name`. The `nameRu` field is populated by `mapUniqueItem` (`poe2api.ts:1030`) via `getUniqueRuName(slug)`. Components covered: `unique-table.tsx` (iter 147), `comparison-dialog.tsx` + `comparative-chart.tsx` + `leveling-uniques-widget.tsx` + `fuzzy-search.tsx` (iter 148). For `LevelingUniquesWidget` which uses the backend `LevelingUnique` type (no `nameRu` field), use `getUniqueDisplayName(unique.name, locale) ?? unique.name` instead — coverage is partial because poe2db slugs don't always match the curated backend names. For `FuzzySearch`, keep the EN name as a `nameAlt` fuse.js key (weight 0.25) so users can search across both languages. For `PairComparisonDialog` (currency pairs), re-derive labels at render time via `getCurrencyDisplayName(pair.currency1Id, locale)` — do NOT trust stored `pair.label` (KI-34 lesson: it's frozen at add-time).
