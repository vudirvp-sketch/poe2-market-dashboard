...
> **Last updated:** 2026-07-12 (iter 134 — TD-3 pipeline_cache optimization SHIPPED. `_refresh()` now populates `pipeline_cache` for the live `/triangular` route via `compute_triangular_cycles(pipeline_cache=...)`, eliminating the doubled `find_triangular_arbitrage` compute cost. Extracted `build_cross_rate_warning` as a shared pure helper (DRY parity guarantee between refresh path and live route). 19 new pytest regression tests. 1455 pytest green in both UTC and UTC+5, 0 regressions. Backend-only. No frontend changes.)
...
| ID | Priority | Notes |
|----|----------|-------|
| **TD-3** | P3 | Triangular arbitrage persistence. **Phase 3 SHIPPED iter 129.** **pipeline_cache optimization SHIPPED iter 134** — `_refresh()` now pre-populates `pipeline_cache` for the live `/triangular` route (eliminates doubled `find_triangular_arbitrage` compute cost). **Open:** verify in production logs that rows land in SQLite (check `historical.db` size after 24h). |
...
| **TD-9** | P3 | FlipsTable "Trend" sparkline. **Phase 1 SHIPPED iter 127** — renders REAL `price_history_short`. **Fallback removal** deferred to iter 135+ (verify no fallback path is hit in production logs for 2 iters first — design doc §10 Q5). |
