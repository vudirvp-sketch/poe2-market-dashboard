...
> **Last updated:** 2026-07-13 (iter 150 — Leveling-uniques-widget full RU coverage: backend `nameRu` field added. Curated 4/10 confirmed poe2db RU translations (Polcirkeln → "Полярный круг", Megalomaniac → "Мания величия", Mind of the Council → "Разум Совета", Soul Tether → "Оковы души"); 6/10 have `name_ru=None` (no poe2db RU page under matching slug). Frontend `leveling-uniques-widget.tsx` switched from fragile `getUniqueDisplayName(name, "ru")` slug-lookup (~1/10 coverage) to direct `unique.nameRu` field. +9 pytest + +1 jest. **1527 pytest + 641 Jest + tsc clean + 0 new ESLint warnings.**)
...
| **TD-6** | P2 | Translation alignment with poe2db official + unique-items RU support. **Phase 1 SHIPPED iter 146** (`--apply-audit` + 32 KI-32 drift items). **Phase 2 SHIPPED iter 147:** `--fetch-unique-ru` + `--apply-unique` flags; 445 unique items added. **Phase 2 follow-up SHIPPED iter 148:** extended `nameRu` rendering to `comparison-dialog`, `comparative-chart`, `leveling-uniques-widget`, `fuzzy-search`; closed KI-34. **Phase 2 follow-up SHIPPED iter 150:** added curated `nameRu` field to backend `LevelingUniqueData` model (4/10 items populated from poe2db RU cache; 6/10 `None` — frontend falls back to EN). Widget uses `unique.nameRu` directly instead of fragile `getUniqueDisplayName(name, "ru")` slug-lookup. **Phase 3:** re-audit cycle (monthly `--audit` + `--apply-audit` for currency; monthly `--fetch-unique-ru` + `--apply-unique` for unique items; periodic re-check of leveling-uniques table for new poe2db RU matches). |
...
| Unique items (weapons, armour, accessories) show English names even when locale=ru | **TD-6 phase 2 + follow-ups** (SHIPPED iter 147 + iter 148 + iter 150) — `unique_names_ru`/`unique_names_en` populated (445 items). `mapUniqueItem` in `poe2api.ts:1030` populates `nameRu`. ALL UI components that display unique-item names use `nameRu` when locale=ru: `unique-table.tsx`, `comparison-dialog.tsx`, `comparative-chart.tsx`, `fuzzy-search.tsx` (with EN kept as `nameAlt`), `leveling-uniques-widget.tsx` (via curated backend `nameRu` field on `LevelingUniqueData`, iter 150 — 4/10 items covered). To refresh poe2db-driven translations: `--fetch-unique-ru` + `--apply-unique --confirm` + `python scripts/sync_currency_names_ts.py`. To extend leveling-uniques coverage: re-run `--fetch-unique-ru` after a poe2db update, then manually update the static table in `backend/economy/leveling_uniques.py:_LEVELING_UNIQUES` for new matches. | `src/lib/poe2api.ts:mapUniqueItem`, `src/components/dashboard/{unique-table,comparison-dialog,comparative-chart,leveling-uniques-widget,fuzzy-search}.tsx`, `backend/economy/leveling_uniques.py:_LEVELING_UNIQUES`, `backend/data/currency_names.json:unique_names_ru/en` |
...
| 6 pytest modules fail on collection (`test_daily_stats_persistence`, `test_market_spreads*`, `test_scheduler`, `test_triangular_cycles*`) | Env-setup issue — `aiosqlite` missing from active venv. Run `pip install -r requirements.txt` (or `pip install aiosqlite`). With aiosqlite installed, the full suite is **1527 pytest green**. Does NOT indicate a regression. | env-only |
...
**UI nameRu rendering pattern (iter 147 + iter 148 + iter 150).** All components that display a `PoeItem.name` for a unique item should use the locale-aware pattern: `locale === "ru" && item.nameRu ? item.nameRu : item.name`. The `nameRu` field is populated by `mapUniqueItem` (`poe2api.ts:1030`) via `getUniqueRuName(slug)`. Components covered: `unique-table.tsx`, `comparison-dialog.tsx`, `comparative-chart.tsx`, `fuzzy-search.tsx` (EN kept as `nameAlt` fuse.js key, weight 0.25), `leveling-uniques-widget.tsx` (iter 150 — uses curated backend `nameRu` field on `LevelingUniqueData`, NOT the slug-lookup). For `PairComparisonDialog` (currency pairs), re-derive labels at render time via `getCurrencyDisplayName(pair.currency1Id, locale)` — do NOT trust stored `pair.label` (KI-34 lesson). **iter 150 lesson:** for static tables where the poe2db slug doesn't always match the curated EN name (e.g. `Polcirkeln Sapphire Ring` vs poe2db slug `Polcirkeln`), prefer a curated `name_ru` field on the backend model over a runtime slug-lookup — slug-lookup coverage was ~1/10, curated field gives 4/10 + an obvious extension path (edit the static table when poe2db adds new RU pages).
