...
> **Last updated:** 2026-07-11 (iter 123 — KI-24 FULLY RESOLVED. All 3 remaining `set-state-in-effect` sites in `dashboard-page.tsx` refactored via the iter 118 "adjust state during render" recipe. Lint 117 → 114, 0 errors. 622 jest green, tsc green.)
...
### None — KI-24 closed iter 123. See "Technical-debt backlog" for low-priority items.

---

## Known Issues — closed (recent)

- **KI-24** (closed iter 123): React Compiler `set-state-in-effect` rule migration — all 10 sites across 7 files resolved (iter 115–123). See recipes below for the patterns. Lint now at 114 warnings (0 errors), all of which are pre-existing `no-unused-vars` / `exhaustive-deps` / etc.
...
| **TD-10** | P4 | `dashboard-page.tsx` line 163: unused `ReferenceCurrency` type import. Line 312: unused `sseStatus` variable. Line 732: `keyboardActions` useMemo exhaustive-deps (missing `TAB_MAP`, `openDetail`). All pre-existing, unrelated to KI-24. |
| **TD-11** | P4 | Repo cleanup: remove `MERGE_INSTRUCTIONS_iter*.md`, `git_commands_iter*.txt`, `DELETIONS.*`, `DELETE_*` placeholder files. ~20+ obsolete files from iter 100–118. |
