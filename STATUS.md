...
> **Last updated:** 2026-07-12 (iter 135 — TD-9 fallback removal SHIPPED. The synthetic `deriveTrendSparklineData(momentum, volatility)` helper was deleted from `flips-helpers.ts`; `getTrendSparklineData` now returns `[]` when no real history exists (≥ 2 points required), and the `Sparkline` component renders an em-dash placeholder (`—`) for empty arrays. `TrendSparklineInput` dropped `momentum` / `volatility` fields. `flipsTrendTooltip` i18n key (en/ru/zh/ko) repurposed from "derived indicator" text to "shows — when no history is available". Stale backend comments referencing the fallback updated. 8 jest tests removed + 4 jest tests updated. 690 jest green, 1455 pytest green in both UTC and UTC+5, 0 regressions. Frontend-only + comment-only backend changes. Closes one of the two oldest deferred items per iter 134 stopping-point brief.)
