...
> **Last updated:** 2026-07-13 (iter 147 — TD-6 phase 2 SHIPPED. Added Stage 7 `--fetch-unique-ru` + Stage 8 `--apply-unique` flags to `scripts/sync_currency_names_from_poe2db.py`. Stage 7 fetches poe2db's unique-item INDEX pages in both RU (`/ru/Unique_item`) and EN (`/us/Unique_item`), parses `<a class="UniqueItem" href="..."><span class="uniqueName">NAME</span></a>` anchors, joins on slug, writes `scripts/.cache/poe2db_unique_names.json`. Stage 8 applies the cache to new `unique_names_ru` / `unique_names_en` sections in `currency_names.json` (idempotent, requires `--confirm`, preserves RU/EN key parity). Ran the pipeline: **445 unique items added** (1 partial `Demigods_Virtue` skipped — RU only, no EN). Updated `scripts/sync_currency_names_ts.py` to emit `UNIQUE_NAMES_RU`/`UNIQUE_NAMES_EN` records + new `enNameToUniqueSlug` / `getUniqueRuName` / `getUniqueEnName` / `getUniqueDisplayName` helpers. Added `nameRu?: string | null` field to `PoeItem` interface (optional, backward-compat). `mapUniqueItem` in `poe2api.ts:1030` now populates `nameRu` via slug lookup. `unique-table.tsx` uses `item.nameRu ?? item.name` when `locale === "ru"`. 26 new tests added (8 parser + 2 fetch join + 8 apply_unique + 4 apply-unique CLI + 2 fetch-unique-ru CLI + 3 currency_names_ru). **1518 pytest green** (was 1492) + 690 Jest green + tsc clean + 0 new ESLint errors. Addresses 2nd half of user complaint about missing RU translation for unique items.)
...
| **TD-6** | P2 | Translation alignment with poe2db official + unique-items RU support. **Phase 1 SHIPPED iter 146** (`--apply-audit` + 32 KI-32 drift items). **Phase 2 SHIPPED iter 147:** `--fetch-unique-ru` + `--apply-unique` flags; 445 unique items added to `unique_names_ru`/`unique_names_en`; `mapUniqueItem` populates `nameRu`; `unique-table.tsx` uses `nameRu` when locale=ru. **Phase 3:** re-audit cycle (monthly `--audit` + `--apply-audit` for currency; monthly `--fetch-unique-ru` + `--apply-unique` for unique items to pick up new items). |
...
| Unique items (weapons, armour, accessories) show English names even when locale=ru | **TD-6 phase 2** (SHIPPED iter 147) — `unique_names_ru`/`unique_names_en` sections now populated (445 items). `mapUniqueItem` in `poe2api.ts:1030` populates `nameRu` field. `unique-table.tsx:135` uses `item.nameRu` when locale=ru. To refresh: `--fetch-unique-ru` + `--apply-unique --confirm` + `python scripts/sync_currency_names_ts.py`. NOTE: only `unique-table.tsx` uses `nameRu` currently; other components (comparison-dialog, comparative-chart, leveling-uniques-widget) still show EN — candidate for iter 148+. | `src/lib/poe2api.ts:mapUniqueItem`, `src/components/dashboard/unique-table.tsx:135`, `backend/data/currency_names.json:unique_names_ru/en` |
