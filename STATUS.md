...
> **Last updated:** 2026-07-12 (iter 133 — KI-26-audit COMPLETE + KI-27 closed. Audited all 17 `replace(tzinfo=timezone.utc)` call sites in `backend/`: 3 UNSAFE → fixed in `lifecycle.py` + `arbitrage/triangular.py` (KI-27); 14 SAFE (read naive UTC from SQLite/POE2Scout). 4 new pytest regression tests. 1436 pytest green in both UTC and UTC+5, 0 regressions. Backend-only. No frontend changes.)
...
- **KI-27** (closed iter 133): KI-26-audit found 3 UNSAFE `replace(tzinfo=timezone.utc)` call sites on potentially-naive LOCAL datetimes — same latent bug class as KI-26. Fixed by switching to `astimezone(timezone.utc)` (same fix pattern as KI-26 in `triangular_cycles._safe_snapshot_age_sec`). Sites: (1) `backend/economy/lifecycle.py:days_since_reference` — `current = now or datetime.now(timezone.utc)` could be naive if caller passes `now` from `datetime.now()` without tz; (2) same site — `reference = self._reference_date()` could be naive if `patch_reset_date` was set from a naive API request body (pydantic accepts naive ISO strings); (3) `backend/arbitrage/triangular.py:_compute_confidence` — `snapshot_time` parameter could be naive if a test or future caller passes `datetime.now()` without tz. 4 new pytest regression tests (2 in `test_lifecycle.py::TestNaiveDatetimeTimezoneHandling`, 2 in `test_triangular.py::TestComputeConfidenceNaiveDatetime`) — each compares the result against an explicit `naive.astimezone(timezone.utc)` conversion. 1436 pytest green in both UTC and UTC+5.
- **P10 / Phase-2** (closed iter 132): `GoldMapRoiTrendChart` SVG trend chart wired into `gold-map-roi-tab.tsx` below the calculator. Consumes iter-129 `GET /api/v1/arbitrage/triangular/history` via proxy `GET /api/flipper/triangular/history`. Shows best-cycle `raw_profit_pct` per timestamp (one line, deduped to highest profit per 5-min bucket) with Days selector (1/7/14/30/90, default 7). Pure helper `pickBestPerTimestamp(points)`. 13 new jest (5 helper + 8 component), 698 total green.
- **TD-5 / Phase-4** (closed iter 131): `daily_stats` persistence shipped — new SQLite table + pure helpers + `HistoricalStore.write_daily_stats_batch`/`read_daily_stats` + `GET /api/v1/items/{item_id}/daily-stats` route with lazy-fetch provider fallback + `DataScheduler.refresh_daily_stats` hourly job + `scripts/backfill_daily_stats.py`. Uses `INSERT OR REPLACE` (POE2Scout may revise a day's candle); 365-day retention. Write happens in TWO places (route lazy-fetch + scheduler), NOT inside `_refresh()`. 56 new pytest, 1432 total green.
- **KI-26** (closed iter 130): `_safe_snapshot_age_sec` in `backend/economy/triangular_cycles.py` used `replace(tzinfo=timezone.utc)` for naive datetimes — relabels wall-clock as UTC without converting. Fixed by switching to `astimezone(timezone.utc)`. 1376 pytest green in both UTC and UTC+5. **Audit (KI-26-audit) deferred** to iter 133+.
...
| ID | Priority | Notes |
|----|----------|-------|
| **TD-3** | P3 | Triangular arbitrage persistence. **Phase 3 SHIPPED iter 129.** **Open:** verify in production logs that rows land in SQLite (check `historical.db` size after 24h). **Performance note (next iter candidate):** compute runs `find_triangular_arbitrage` (~30-60s) inside `_refresh()` — DOUBLES compute cost (one in `_refresh`, one in live `/triangular` route's first-request cache). Refactor to populate `pipeline_cache` from `_refresh()` if bottleneck. Backend-only, no UI impact. |
...
| **TD-9** | P3 | FlipsTable "Trend" sparkline. **Phase 1 SHIPPED iter 127** — renders REAL `price_history_short`. **Fallback removal** deferred to iter 134+ (verify no fallback path is hit in production logs for 2 iters first — design doc §10 Q5). |
