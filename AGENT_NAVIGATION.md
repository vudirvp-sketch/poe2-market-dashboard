...
> **Single entry point** for codebase navigation. Updated 2026-06-25 (iter 81 — `useFlipperBackend` hook extracted as Stage 1 of `useDashboardData` refactor).
...
| `src/components/dashboard/dashboard-page.tsx` | Parent wiring component, **1197 lines** (iter 81: −35 lines via `useFlipperBackend` extraction). 6 presentational subcomponents extracted in iter 71-73. | Optional follow-up: `useDashboardData` hook for the remaining ~200 lines of useQuery/memo wiring. Stage 1 done iter 81; stages 2-3 (realms/leagues + derived memos) still pending. |
...
34. **`useFlipperBackend` is the single source of truth for dashboard-level flipper status** (iter 81, Stage 1 of `useDashboardData` extraction). The three dashboard-level flipper queries (`/api/flipper/health`, `/api/flipper/phase`, `/api/flipper/events?active_only=true`) MUST go through `useFlipperBackend()` — NOT inline `useQuery` calls in `dashboard-page.tsx`. The hook returns: `flipperBackendOnline` (boolean — true when health is "ok" or "degraded"), `flipperUpstreamReachable` (boolean — true when `health.provider === "reachable"`), `flipperPhaseData` (header badge data), `activeEventsCount` (events button indicator), `flipperHealthData`/`flipperHealthPending`/`flipperHealthError` (raw health probe state, exposed for future loading indicators), `flipperEventsData` (raw events response). Phase and events queries are `enabled: flipperBackendOnline` — they do NOT fire when the backend is offline (matches pre-iter-81 behaviour). Query keys are `QUERY_KEYS.flipperHealth` / `flipperPhase` / `flipperEventsCount` (unchanged). **Stages 2-3 still pending** (realms/leagues queries, then derived memos like exchangePairs filter / optimalPayment merge / currencyCategories). Each stage should be shipped as a separate iter with tsc + jest verification to avoid the "high interdependency risk" flagged in P2-1.
...
| `dashboard-page.tsx` still 1197 lines | Optional follow-up: extract `useDashboardData` hook for remaining ~200 lines of useQuery/memo wiring. Stage 1 done iter 81 (`useFlipperBackend`); stages 2-3 still pending. Not blocking. | — |
