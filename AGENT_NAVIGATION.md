...
> **Single entry point** for codebase navigation. Updated 2026-06-26 (iter 82 — `useRealmsAndLeagues` hook extracted as Stage 2 of `useDashboardData` refactor).
...
| `src/hooks/` | React hooks (16 hooks) | Import from `@lib` |
...
| `src/components/dashboard/dashboard-page.tsx` | Parent wiring component, **1169 lines** (iter 82: −28 lines via `useRealmsAndLeagues` extraction; iter 81: −35 lines via `useFlipperBackend`). 6 presentational subcomponents extracted in iter 71-73. | Optional follow-up: `useDashboardData` hook for the remaining ~180 lines of useQuery/memo wiring. Stages 1-2 done iter 81-82; stage 3 (derived memos: exchangePairs filter / optimalPayment merge / currencyCategories) still pending. |
...
| `dashboard-page.tsx` still 1169 lines | Optional follow-up: extract `useDashboardData` hook for remaining ~180 lines of useQuery/memo wiring. Stages 1-2 done iter 81-82; stage 3 (derived memos) still pending. Not blocking. | — |
